...
with webdriver.Chrome(service=service) as driver:
    ...
    for ind, cookie in enumerate(cookies):
        ...
        age = driver.find_element('id', 'age').text
        skills = driver.find_elements('xpath', '//ul[@id="skillsList"]/li')

        age = int(age.split(':')[1])
        number_languages = len(skills)

        if age <= min_age:
            min_age = age
            if number_languages > count_languages:
                count_languages = number_languages
                best_hacker = (min_age, number_languages, ind)

    age_hacker, skills_hacker, ind_hacker = best_hacker
    driver.refresh()
    driver.add_cookie(cookies[ind_hacker])
